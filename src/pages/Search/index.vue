            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <!--固定写法： class:iconfont icon-long-arrow-down-->
...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      // 可有可无的数据可以不传输给服务器，优化性能
      // 这个时候可以用undefined，就不会传输给服务器了
      this.searchParams.categoryName = undefined;
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      // 地址栏的数据也要清除。即路由跳转到search
      // this.$router.push({name:'search'})
      // 但是params不应该删
      if (this.$route.params) {
        this.$router.push({ name: "search", params: this.$route.params });
      }
      // console.log(this.$route.params); //undefined
    },
    removeKeyword() {
    ...
    trademarkInfo(trademark) {
      // console.log('我是父组件',trademark.tmName);
    ...
    attrInfo(attr, attrValue) {
      // 整理数据（看word文档）"属性ID:属性值:属性名"
      let props = `${attr.attrId}:${attrValue}:${attr.attrName}`;
      // 把该字符串放入searchParams.props这个数组里面
      // 数组去重
    ...
    changeOrder(flag){
      ...
      if(flag == orginFlag){
        ...
        // this.searchParams.order = `${flag}:${orginSort=='desc'?'asc':'desc'}`
  ...
  watch: {
    $route() {
      ...
      // 再次发起请求
      this.getData();
      // 每一次请求完毕，应该把相应的1,2,3,级分类的id置空，让它接收下一次的相应的1,2,3级id
