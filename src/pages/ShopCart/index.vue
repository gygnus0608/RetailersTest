      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteCartListById(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // console.log('区分点击的按钮',type,'区分点击的数量',disNum,'区分点击的产品',id);
      // disNum：形参，代表变化量
      switch (type) {
        case "add":
          disNum = 1;
        ...
        case "minus":
          ...
          break;
        case "change":
          // 如果有非数字的情况或者值为负数,disNum=0(不发生变化)
          if (isNaN(disNum) || disNum < 1) {
            //这里的disNum指输入的值
            disNum = 0; //这里的disNum值变化量
          } else {
            // 变化量=输入的值-原先的值
            disNum = parseInt(disNum) - cart.skuNum;
          }
      }
      // 派发action：重新向服务器发请求，改变购物车某商品(skuId)的数量(skuNum)
      try {
        // 代表修改成功
        // 这边将disNum作为skuNum传过去，后端做的工作是在原来的基础上加减，加减的值又传过去的值决定
    ...
    async updateChecked(cart, event) {
      ...
      try {
        ...
        // 这里加async await是为了让dispatch先执行然后再执行this.getData()，因为请求时需要事件的，不加异步的话this.getData()会先执行
        this.getData();
      } catch (error) {
        alert(error.message);
      }
